...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $withdraw_amount = $_POST['withdraws'];
    $date = $_POST['date'];
    ...
    if ($account_id) {
        $result = $net_amount - $withdraw_amount;

        $stmt = $conn->prepare("INSERT INTO withdraw (account_id, withdraw_amount, account_number, date, result) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("iissi", $account_id, $withdraw_amount, $account_number, $date, $result);

        if ($stmt->execute()) {
            $stmt_update = $conn->prepare("UPDATE account SET net_amount = ? WHERE account_id = ?");
            $stmt_update->bind_param("is", $result, $account_id);
            if ($stmt_update->execute()) {
                echo "withdraw successful and account updated!";
            } 
            else {
...
<body>
    ...
    <form action="withdraw.php" method="post">
        <div class="mb-3">
            <label for="withdraw">ถอนเงิน:</label>
            <input type="number" id="withdraw" name="withdraw" class="form-control" required><br><br>
        </div>
